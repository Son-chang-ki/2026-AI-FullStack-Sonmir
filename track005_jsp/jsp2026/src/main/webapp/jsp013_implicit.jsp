...
<body>
   <div class="container card my-5">
      ...
      <pre class="alert alert-info">
       ...
       <h4>002. request(요청) - getParameter</h4>
	       <pre class="alert alert-info">
	        1. 처리하는 해결사 : action = "jsp013_1.jsp"
	        2. 요청방식 	  : 주소표시창 노출
	        3. 보관용기이름   : query = getParameter("name")
	        4. 출력내용 :      검색어 : 입력한 값
	      </pre>
       ...
       <hr/> <!-- 라인 넣기 -->
       <h4>003. response(응답)</h4>
       <pre class="alert alert-info">
         1. http 요청에 대한 응답 저장하는 객체
         2. jsp에서 다른 페이지로 이동
           1) sendRedirect      - 브라우저 url 을 변경 O, request / response 객체가 유지 안됨.
           2) RequestDispatcher - 위임(요청전달)
           					      브라우저 url 을 변경 X, request / response 객체가 유지 됨.
       ...
       //ver-2 
       // jsp013_3.jsp .
       // request.getRequestDispatcher("jsp012_milk.jsp") 이용해서 여기서 처리해 주세요...
       // response.sendRedirect("jsp013_3.jsp"); 
       %>
       
       <pre>
          jsp013_implicit.jsp (1) : 나이 입력받는 폼     →  jsp013_4.jsp  처리 (2)
           ...
           <pre class = "alert alert-success"> 
	           1. application(웹애플리케이션이 실행되고 있는동안, 서버가 꺼질때까지 유지) 
	                > session (특정브라우저와 연결된 세션, 브라우저 종료시 소멸)
	                   > request(요청 : form, a 태크 요청) 
	                      > page(해당jsp 페이지에서만)
	           2. 객체.setAttribute("속성", "값")   / 객체.getAttribute("속성")
            </pre>
            <%
              application.setAttribute("name", "D.application - 웹애플리케이션이 실행");
       ...
       <pre class = "alert alert-success"> 
         ...
         ** 해결방안
         ...
         <h4 class="card-haeder">1. ERROR 404 </h4>
	      <p><a href="jsp013_6_error.jsp" class="btn btn-danger">NO PAGE</a></p>
	      
	      <h4 class="card-haeder">2. ERROR 500 </h4>
	       <p><a href="jsp013_7_500.jsp" class="btn btn-danger">NO PAGE</a></p>
	        <%-- <%   int i=40/0;  %>   --%>
