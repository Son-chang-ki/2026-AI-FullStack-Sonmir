 ...
 %>  </p>
      ...
      <pre>
       ...
       <c:choose>
         ...
         <c:otherwise> 1,2,3 이 아니다. </c:otherwise>
       </c:choose>
       
       <h4 class="card-header"> 2) forEach </h4>
       ...
       <c:forEach   var="i"   items="${numbers2}"  varStatus="status">
          ${status.first}   -    ${status.index}     -    ${1} <br/>
       </c:forEach>
		...
		<c:forEach   var="i"   items="${season}"> ${i} 입니다. <br/>    </c:forEach>
