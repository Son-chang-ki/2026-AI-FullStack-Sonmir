...
-- ------------------------------------------------------------
-- ------------------------------------------------------------ [QUESTION]
-- subqury ==> selete 안에 selete 구문 사용
-- --------------------------------------------------------
-- --------------------------------------------------------
...
-- select  *
-- from    emp
-- where   sal > ( 사원이름이 JONES의 급여)  where  sal > ( ename = 'JONES')
-- where  sal > (select  sal  from  emp  where  ename = 'JONES' order by sal);
select  *
from  emp
...
-- where hiredate < (select  hiredate from emp  where ename = 'scott' order by hiredate);
select *
from emp
...
-- where deptno = 20 and sal > (select sal from emp e  where e.deptno = 20 order by sal);

select EMPNO , ENAME, JOB  ,SAL , d.DEPTNO , DNAME  , LOC 
from emp e   join dept d using(deptno)
where e.deptno = 20 and sal > (select avg(sal) from emp )  -- 조건 정리
order by e.empno desc; -- 정렬
-- +-------+-------+---------+------+--------+----------+--------+
...
-- - 서브쿼리의 결과가 한개이상이 반환될때  in, ANY(SOME), ALL , EXISTS 을 사용할수 있다.
-- -                        한개일때                  = 을 사용한다.
...
order by num;    -- 결과 : 1,2,3,4    -- 5 보다 작아야 한다

-- Q2) 
select   num 
from     atest  
where    num > any(    select num  from atest  where num in(3,4,5) )  -- 1줄 3보다 크다
order by num;  -- 결과 : 4,5,6


-- Q3) 
select   num 
from     atest  
where    num < all(    select num  from atest  where num in(3,4,5) )  -- 모두 만족해야 함. 3보다 작아야 함
order by num;  -- 결과 : 1,2
...
select   num 
from     atest  
where    num > all(    select num  from atest  where num in(3,4,5) )  -- 모두 만족해야 함. 5보다 커야함
order by num;  -- 결과 : 6
