...
-- SELECT   검색필드1, 검색필드2,,,
-- FROM    테이블명
-- WHERE where_condition 생갹가능
-- ORDER BY  기준필드  [ASC (오름차순, 기본, 생략가능)| DESC](내림차순)
-- LIMIT offset;
...
create table select_userinfo select * from userinfo; -- 데이블복사 + 자료형
...
delete from selet_userinfo where no >=5;  -- 5번이상 데이터 삭제
...
from     select_userinfo
order by age desc
limit 1, 2;  -- 어디서부터, 몇개 (최신부터, 2개) 55 44


create table select_emp select * from emp; -- 데이블복사 + 자료형
desc select_emp;
...
desc select_emp;
select   * from     emp    where  ename  and  mrl and sal;
...
-- 실습 12 emp테이블에서 1순위는 부서번호를 (deptno)를 기준으로 오름차순, 그중에서도 급여를 기준으로(sal) 내림차순으로 정렬하시오.
select  *
from    emp
...
select  
empno 'EMPLOYEE_NO', ename 'EMPLOYEE_NAME', job 'JOB', mgr 'MANAGER', 
hiredate  'HIREDATE', sal 'SALARY' , comm 'COMMISSION', 
dept 'DEPARTMENT_NO'
from    emp
-- where
