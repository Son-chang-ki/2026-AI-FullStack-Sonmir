...
use mbasic;
desc userinfo;
create table userinfo_select select * from userinfo;    -- 테이블 복사해서 만들기
...
set sql_safe_updates = 0;  -- 여러 줄 안전삭제 옵션끄기
delete from userinfo_select where no >= 4;   -- 4번 이후로 데이터 삭제
update userinfo_select set age=null where no=3;  -- 3번 데이터 null 값으로 수정
desc userinfo_select;

-- create table userinfo_select select * from userinfo where 1=2;    -- 구조 복사
 select * from userinfo where 1=2;  -- 1=2 같니? 해당하는 값 없어 구조만 복사됨
...
and name not like 'e%'; 		 -- 이름의 두번째 글자가 e가아닌 데이터를 조회하시오
...
select ceil(1.1) `올림 2`, floor(1.9) `내림 1`, round(1.5) `반올림`, mod(10,3) `나머지 1` 
from dual;
...
-- mysql>
-- mysql> -- 5. 찾기 3  - substr ( 문자열, 시작위치, 가져올 문자 갯수 )  - 문자열일부분 추출
...
-- mysql>
-- mysql> -- 7. 공백빼기 - trim ==> 중간 공백은 X 앞 뒤 공백만 제거 됨
select trim(' a b c ')
from dual;
-- +-----------------+
-- | trim(' a b c ') |
...
insert into fn_select_userinfo2 (name, age, email) values ('aaa' , 11 ,'aaa@gmail.com');
insert into fn_select_userinfo2 (name, age, email) values ('bbb' , 22 ,'bbb@gmail.com');
insert into fn_select_userinfo2 (name, age, email) values ('ccc' , 33 ,'ccc@gmail.com');
insert into fn_select_userinfo2 (name, age, email) values ('ddd' , 44 ,'ddd@gmail.com');
insert into fn_select_userinfo2 (name, age, email) values ('abc' , 55 ,'abc@gmail.com');
insert into fn_select_userinfo2 (name, age, email) values ('bca' , 66 ,'bca@gmail.com');
...
-- length   (개수) upper  대문자/ lower  소문자  left( 문자열 왼쪽, 갯수) right( 문자열 오른쪽, 갯수)
...
-- 3. 유저이름의 aaa인 유저를 찾아서 'aaa 1등'으로 변경하시오. replace (문자열, 찾아서, 바꾸기)
...
-- 4. 다음과 같이 직업을 출력하시오.   (concat)   concat ( 문자열, 문자열 ) - 문자열연결
...
-- concat ( 문자열, 문자열, 문자열 ) - 문자열연결  repeat (문자열, 몇번)  substr( 문자열, 시작, 갯수 )  - 문자열일부분 추출
-- lpad(문자열, 전체자리수, 특정문자) rpad(문자열, 전체자리수, 특정문자)
...
use mbasic;
create table fn_select_userinfo3  select * from fn_select_userinfo2 where 1=2;
alter table fn_select_userinfo3   modify   no int not null  auto_increment primary key;

insert into fn_select_userinfo3 (name, age, email) values ('aaaaa' , 11 ,'aaa@gmail.com');
insert into fn_select_userinfo3 (name, age, email) values ('bbaab' , 22 ,'bbb@gmail.com');
insert into fn_select_userinfo3 (name, age, email) values ('ccaac' , 33 ,'ccc@gmail.com');
insert into fn_select_userinfo3 (name, age, email) values ('ddddd' , 44 ,'ddd@gmail.com');
insert into fn_select_userinfo3 (name, age, email) values ('abc'   , 55 ,'abc@gmail.com');
insert into fn_select_userinfo3 (name, age, email) values ('baaca' , 66 ,'bca@gmail.com');

select * from fn_select_userinfo3;
-- +----+-------+-----+---------------+
-- | no | name  | age | email         |
-- +----+-------+-----+---------------+
