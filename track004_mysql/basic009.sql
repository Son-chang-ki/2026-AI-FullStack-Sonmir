...
select date_format('2026-05-26 14:37:13', "%Y-%m-%d");   -- 2026-05-26
select date_format(now(), "%Y-%m-%d");   -- 2026-05-26
select date_format(now(), "'%Y-%m-%d %H:%i-%s");  -- '2026-05-26 14:44-33

-- 참고사항)
-- https://www.w3schools.com/sql/func_mysql_date_format.asp

-- 4.  100일전/ 100일 후  - date_add 
select date_add( now(), interval -10 day);  -- 2026-05-16 14:46:08
select date_add( now(), interval 10 day);   -- 2026-06-05 14:46:34
select date_add( now(), interval 3 hour);   -- 2026-05-26 17:47:12
...
select timestampdiff(hour, "2026-05-25", "2026-05-26");  -- 24 시간alter
...
use mbasic;
create table data_userinfo select * from userinfo where 1=2;
desc data_userinfo;
alter table data_userinfo add date datetime default current_timestamp;  -- date 필드 추가
alter table data_userinfo modify no int not null auto_increment primary key;  -- 기본키 추가
desc data_userinfo;
alter table data_userinfo rename date_userinfo;
desc date_userinfo;
-- +-------+--------------+------+-----+-------------------+-------------------+
...
-- +------+---------------------+
-- 힌트)
-- select  #2) name, date  date_add( now(), interval 10 day);
-- from    #1) date_userinfo
-- where   #3) '2022-11-01' 이전에 회원가입한 (조건식)
-- group by -- 
-- having -- 
-- order by --  정렬
...
from date_userinfo 
where date >='2022-12-01' and date <= '2022-12-30'; 
...
select datediff('2026-06-06', now() );   -- 11

-- > 3. 현재날짜에서 5개월 더해 출력하시오.
select date_add(now() , interval 5 month);  -- 2026-10-26 15:48:30
...
select date_add(now(), interval 5 day); -- 결론 : 수동으로 5월31일 6-1=5  ==>  2026-05-31 15:53:14
select date_add(now(), interval (6- weekday(now())) day); -- 내일이와도 가장 오는 일요일

select date_add(now(), interval (6-1) day); 
...
-- 16:45:02	delete  from  if_userinfo	
...
-- +----+--------+-----+------+------+----------+
-- 6 rows in set (0.00 sec)
select no, name, age, sex, sns, if(age<=19, '미성년자', '성인')`성인여부`
from if_userinfo;
...
SELECT 
    CONUT(IF(sns = 'n', 1, NULL)) AS sns수신거부수
FROM
    if_userinfo;
