...
-- FROM    테이블명
-- WHERE where_condition  
-- ORDER BY   [ASC (오름차순, 기본, 생략가능)| DESC (내림차순)] ==> 정렬
-- LIMIT offset;
...
-- |1   | first  |  11 | NULL |  100 |  100 |   99 | A    | n    |   ==> | 를 , 로 수정하기 (메모장에서 붙여놓고 ctrl + h)
-- |  2 | second |  22 | m    |   89 |   92 |   78 | B    | y    |
-- |  3 | third  |  33 | m    |   90 |   92 |   97 | A    | y    |
-- |  4 | fourth |  44 | f    |   40 |   42 |   67 | C    | n    |
-- |  5 | fifth  |  55 | f    |   89 |   86 |   99 | B    | y    |
...
-- +------+----------+----------+----------+----------+
-- 3 rows in set (0.00 sec)
select    ban,  sum(age) `나이평균`, avg(age) `최고나이`, avg(age) `최저나이` 
from      group_userinfo
group by  ban;
...
-- +-------------------+--------------+----------+
-- | sum(distinct sal) | sum(all sal) | sum(sal) |  -- distinct   ==> 중복제거 의미함
-- +-------------------+--------------+----------+
-- |             24775 |        29025 |    29025 |
-- +-------------------+--------------+----------+
-- 1 row in set (0.00 sec)
...
-- --------------------------------------------------------
-- -- 연습문제-4
-- -- emp 테이블의 레코드 갯수(count)를  조회하시오. -- 집계함수는 null 빼고 계산
select count(*) `전체레코드`, count(empno) `모든데이터`, count(mgr) `null 빼고 계산`  -- 14 / 14 / 13
from  emp;
...
select  count(*) 
from    emp
group by  dept having dept=30;  -- 집계 후 필터링
...
from    emp
where   dept = 10;   -- 집계전 필터링


select   max(sal) `부서10 최대급여 `
from     emp
group by dept 
having   dept= 10;   -- 집계 후 필터링
-- +-----------------+
-- | 부서10 최대급여    |
-- +-----------------+
-- |            5000 |
...
-- --------------------------------------------------------
-- -- 연습문제-17
-- -- emp 테이블에서  각 부서의 직책별 평균급여를 구하되 그 평균급여가 2000이상인 그룹만   조회하시오. 
select dept, job, avg(sal) `평균급여` 
from   emp
group by dept, job
having avg(sal) >= 2000
...
-- 13:10:47	부서의 직책별 평균급여를 구하되 그평균급여가 2000이상인 그룹만   조회하시오. 
-- -- -- 다음코드를 작성해보고 오류가 난다면 오류가 나는 이유를 적으시오.	
-- Error Code: 1064. You have an error in your SQL syntax; check the manual that corresponds to your MySQL server version for the right syntax to use near 
-- '부서의 직책별 평균급여를 구하되 그평균급여가 2000이상인 ê' at line 1	0.000 sec
