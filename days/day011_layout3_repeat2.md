...
-2. eclipse 열어서 클래스명 : Day011

if버젼에 해당하는 다음에 연결해서 문제를 작성하시오. 문자를 한개 입력받아 a이면 apple , b이면 banana, c이면 coconut

switch버젼에 해당하는 다음에 연결해서 문제를 작성하시오. 문자를 한개 입력받아 a이면 apple , b이면 banana, c이면 coconut
...
■ 복습문제  - 빈칸채우기   
    1. 레이아웃을 구성하는 대표적인 방식은 무엇인가?    (  float    ,   display    ,  position    )
   ...
   5.  브라우저에 고정되도록 만드는 방법은?  ( display : fixed )
   6.  a태그의 링크를 확대하려고 한다. 사용해야하는 방법은?   display : ( block )
   7.  li태그들을 가로방향으로 배치하려고 한다.  width를 줘야하는데 사용해야하는 방법은?  ( display inline-block )
   8.  header 안에서 로고와 내비게이션을 양쪽 끝에 배치하고, 콘텐츠 간 간격을 일정하게 유지하려면 어떤 레이아웃 방식을 사용해야 하는가?  ( flex space-between  )
