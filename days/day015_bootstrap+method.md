...
■ 1. 복습문제 - 노트 준비 ~09:40
■ 1. html + css
html + css 익히느라 수고하셨습니다~! 내일은 바로 부트스트랩 진행할께요~~!
...
1. __________ >
   - 전체 레이아웃을 감싸는 박스  <div class="continer bg-info p-3"></div>
   - 웹 페이지의 최대 너비를 관리  <div class="continer-fluid bg-info p-3"></div>
   - `container`(고정 폭) / `container-fluid`(가변 폭)

2. __<div class="row bg-info p-3"></div>________
   - `container` 안에서 줄(행)을 만드는 역할   

3. <div class="col-sm-12 bg-info p-3"> </div>______col-디바이스 사이즈 -칸수
   - `row` 안에서 칸(열)을 만드는 역할  
   - 기본 단위는 **12칸**   

4. 4칸으로 균등하게 레이아웃을 지정하려고 한다. 줘야하는것은?   
   <div class="col-sm-3 bg-success p-3"> 3칸 </div>
   <div class="col-sm-3 bg-info p-3"> 3칸 </div>
   <div class="col-sm-3 bg-waring p-3"> 3칸 </div>
   <div class="col-sm-3 bg-light p-3"> 3칸 </div>

5. 전체 레이아웃을 감싸는 박스(여백있음) ,  선주기, 두께는 3으로, 둥근모서리 , 좋아하는 색상 , 배경초록색, 글자 하얀색
<div class="container   boder boder-3, rounded, bg-success text-white"> </div>
