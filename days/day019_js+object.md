...
1-2. 태그 선택자 사용시 -  documenet.getElementsByTagName("태그명")
1-3. css 표현 선택자 사용시  - document.querySelector("css표현방법")
...
2-3. 클릭시 prompt이용해서 당신의 이름묻기
        test.onclick = function(){  let name = prompt("name")  };
...
<script>   
    window.addEvenetListener("load" , function(){
        let test=  document.querySelector("#test")
        test.onclick = function(){  let name = prompt("name"); alert(name);  };
    })
</script>  
