...
웹페이지의 배경을 하늘색으로 바꾸고 싶다. 어떤 속성을 써야 할까? ____________
제목 글자를 초록색으로 바꾸려면? ____________
본문 글자의 크기를 18px로 지정하려면? ____________
메뉴 글자를 오른쪽으로 정렬하려면? ____________
링크에 밑줄을 없애려면? ____________
글꼴을 'Times New Roman'으로 바꾸려면? ____________
강조하고 싶은 단어를 굵게 표시하려면? ____________
이미지를 가로 400px로 줄이고 싶다. 어떤 속성? ____________
박스 바깥쪽에 20px 여백을 주려면? ____________
버튼 안쪽에 8px 여백을 주려면? ____________
카드 요소에 2px 점선 테두리를 주려면? ____________
프로필 사진 모서리를 둥글게 50%로 만들려면? ____________
박스에 살짝 번지는 그림자 효과를 주려면? ____________
버튼에 마우스를 올렸을 때 부드럽게 색이 바뀌도록 하려면? ____________
정리문제 (2)
...
숫자를 한개입력받아
1이면 1이다, 2이면 2이다, 3이면 3이다 if로 작성
