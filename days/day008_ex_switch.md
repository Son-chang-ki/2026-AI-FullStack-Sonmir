...
### ■3. Todo2 : java CONTROL - SWITCH
 - 제어문
   프로그램 코드 실행흐름 :  위 → 아래, 왼쪽 → 오른쪽
    제어문은 개발자가 원하는 방향으로 변경할 수 있도록 도와줌.
    종류 조건문 : if, switch 반복문 : for, while, do while 제어키워드 : break, continue
......
 1. BANK 지난시간에 작성내용
...
■2. java
   ...
   if 버젼 1을 입력받으면 1이다 / 2를 입력받으면 2이다 / 3을 입력받으면 3이다.

   switch 버젼 위의 내용을 switch 버젼으로

   다음 무한반복을 빠져나오는 코드를 적으시오 int a = -1;
