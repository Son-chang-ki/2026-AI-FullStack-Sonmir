...
public class BoardServiceImpl implements BoardService {
	...
	public int insert(BoardDto dto, MultipartFile file) {
		...
		if(!file.isEmpty() ) {
		 ...
		 catch (UnknownHostException e) {e.printStackTrace();} //#1// 실행한 줄 수 1
	...
	@Override public int edit(BoardDto dto  , MultipartFile file) { 
		...
		if(find.getBpass().equals(dto.getBpass()) ) { //글 번호의 비번과 사용자가 입력한 비번이 같은지 확인
			...
			if(!file.isEmpty() ) {
				...
				try {	file.transferTo(demp); } // #2.파일옮기기
				catch (IOException e) { e.printStackTrace(); }
			}
			dto.setBfile(fileName); // #3. 파일명셋팅
			result = dao.update(dto); 
			}
		return result; // 해당 글번호의 비밀번호와 유저가 입력한 비번		
	   } // 비번 맞으면 수정
