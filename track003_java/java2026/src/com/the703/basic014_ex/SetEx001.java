...
class UserInfo2Dto {
	...
	@Override public String toString() 
	  { return "UserInfo2Dto [no=" + no + ", name=" + name + ", age=" + age + "]"; }
...
public class SetEx001 {
	public static void main(String[] args) {
		...
		for (UserInfo2Dto u: users) 
		   {System.out.println(u.getNo() + "-" + u.getName() + "-" + u.getAge()); avg += u.getAge(); }  //유저 나이 누적

		System.out.println("> 찾을 유저이름 : "); find = scanner.next();
		//Q. 만약 유저의 이름이 find 와 같다면 ~! 유저출력
		// if(만약 유저의 이름이 find 와 같다면 ~! equals) { 유저출력 }
		for (UserInfo2Dto u: users) { if (u.getName().equals(find))  {System.out.println(u); } break; }
		//  System.out.println(users.contains(new UserInfo2Dto(2, find , 40)) ? "있어" : "없어");  
		//Q. 나이평균
