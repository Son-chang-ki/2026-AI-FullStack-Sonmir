...
class Barista { 
	public void make(Drink dr) { dr.serve();
	// menu[0] = {@serve() , addSugar() }--{---}
    // menu[1] = {@serve() , addLemon() }--{---}
    // menu[2] = {@serve() , addIce() }--{---}
	if (dr instanceof Coffee)     { ((Coffee) dr).addSugar();}
	else if (dr instanceof Tea)   { ((Tea) dr).addLemon();   } 
	else if (dr instanceof Juice) { ((Juice) dr).addIce();   }
	}
}
