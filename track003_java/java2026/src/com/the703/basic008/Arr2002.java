...
public class Arr2002 {
	...
	public static void main(String[] args) {
		...
		arr21[1][0] =40; arr21[1][1] = 50; arr21[1][2] = 60;   // for 줄이기
		...
		//ver-3
		 //int data=10;
		 //for(int kan=0; kan<arr21[0].length; kan++)   { arr21[0][0] = data; data+=10; }
		 //for(int kan=0; kan<arr21[1].length; kan++)   { arr21[1][0] = data; data+=10; }
		 
		//ver-4
		 int data=10;
		 for(int ch=0; ch<arr21.length; ch++)   { //1) 배열명.length   아파트의 .층
			 for(int kan=0; kan<arr21[ch].length; kan++) { //2) 배열명[0].length   아파트 0층 .칸
				 arr21[ch][kan] = data;
				 data += 10;
			 }
		...
		for(int ch1=0; ch1<arr21.length; ch1++ ) {  //2-1 배열의 층수
			for(int kan=0; kan<arr21[ch1].length; kan++) {  // 2-2 배열[층]의 칸수
