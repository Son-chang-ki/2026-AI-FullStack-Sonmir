...
1. S : 한 클래스는 하나만(한 가지 일만/ 쿠키반죽은 반죽만, 쿠키굽기는 쿠기굽기만)
2. O : (수정말고 추가만 가능)새로운 쿠키 쉽게 추가
...
//2. O : 개발폐쇄 (새로운 쿠키 추가가능, 기존코드 건들지 말기)
...
//3. L : 리스코프치환 (어떤 쿠키든 Cookie 인터페이스로 바꿔써도 문제없어요.. 부모 = 자식)
class CookieFactory{ // Cookie cookie 가종 쿠키종류
	public void makeCookie(Cookie cookie) { cookie.make(); } // 어떤쿠키든 여기서 만들 수 있어요...~~
}
//4. I : interface - 너무 많은 기능을 가용하지 말것!! 꼭 필요한 기능(절차)
interface SimpleCookie { void make1(); /* void make2();   void make3();  */}

//5. D : 의존역전 - 어떤쿠키든 가게에서 팔수 있어요..~!
//				 CookieFactory는 구체적인 쿠키가 아니라 Cookie에 의존해야 함
class CookeShop {
...
public class SolidBasic {
	public static void main(String[] args) {
		...
		maker.bakeCookie("초쿄"); maker.bakeCookie("오트밀"); maker.bakeCookie("라즈베리");
