...
public class UtilPaging { 
   ...
   private  int  pagetotal;    //# 3) 총페이지 193/10 19페이지 + 3글 = 20페이지 필요
   ...
   public UtilPaging(int listtotal, int pageNo) { this(listtotal, pageNo, 10, 10); }  //한페이지 10, 하단네비 10 
    ...
    public UtilPaging(int listtotal, int pageNo , int onepagelist , int  bottomlist) { //페이지크기와 하단네비크기지정
      ...
      this.pagetotal   = (int) Math.ceil(this.listtotal/ (double)onepagelist); // 총페이지 193/10 19페이지 + 3글 = 20페이지
      // 193/10 ==> 19.3  ==> 올림  ==> 20
      // 200/10 ==> 20.0  ==> 올림  ==> 20
